 ...
 * @line: Buffer with the line of input from user
 * @env: Environment
...
int launch_prog(char **args)
{
	...
	else
	{
		...
		} while (!WEFEXITED(status) && WIFSIGNALED(status));
	}
	(void)wpid;
	return (1);
}
 ...
 * @args: Arguments passed from prompt
 * Return: 1 if builtins exit, 0 if they don't
 */
